...
async def test_sharpe_agent_basic_positive(
    ...
    assert pytest.approx(result['value']) == expected_sharpe, \
           f"Agent 'value' ({result['value']:.2f}) does not match expected Sharpe ({expected_sharpe:.2f})"
    assert pytest.approx(result['details']['sharpe_ratio']) == expected_sharpe, \
           f"Agent 'details.sharpe_ratio' ({result['details']['sharpe_ratio']:.2f}) does not match expected Sharpe ({expected_sharpe:.2f})"
