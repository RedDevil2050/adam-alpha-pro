...
async def mock_dependencies():
    ...
    with patch('backend.agents.esg.esg_score_agent.fetch_esg_data', new_callable=AsyncMock) as mock_fetch, \
        ...
        # Use AsyncMock for the tracker instance as its methods might be awaited
        mock_tracker_instance = AsyncMock()
        # Explicitly mock methods if needed, though AsyncMock handles awaited calls
        # mock_tracker_instance.track_agent_start = AsyncMock()
        # mock_tracker_instance.track_agent_end = AsyncMock()
        # mock_tracker_instance.track_agent_error = AsyncMock()
