...
async def run(symbols: list) -> dict:
    ...
    for sym in symbols:
        ...
        results[sym] = {"verdict": cycle[0].get("verdict"), "score": cycle[0].get("score")}

    # Portfolio summary: average score, count of BUYs
    scores = [v["score"] for v in results.values()]
    avg_score = sum(scores)/len(scores) if scores else 0.0
    buy_count = sum(1 for v in results.values() if v["verdict"] in ("STRONG_BUY","BUY"))
    ...
    result = {
        ...
        "value": len(symbols),
        "details": {"avg_score": avg_score, "buy_count": buy_count, "per_symbol": results},
        "score": avg_score,
