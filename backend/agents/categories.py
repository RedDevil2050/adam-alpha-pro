...
class CategoryManager:
    ...
    async def get_category_agents(cls, category: CategoryType) -> List[Type]:
        ...
        for agent_name in cls._agent_registry.get(category, []):
            try:
                module = importlib.import_module(f"backend.agents.{category.value}.{agent_name}")
    ...
    @classmethod
    async def execute_category(cls, category: CategoryType, symbol: str, 
    ...
    @classmethod
    def validate_category_result(cls, category: CategoryType, results: List[Dict]) -> bool:
        """Validate category execution results"""
        if not results:
            return False
        metadata = cls.CATEGORY_METADATA[category]
        if metadata.required and len(results) < len(cls._agent_registry[category]) * 0.5:
            return False
        return True
