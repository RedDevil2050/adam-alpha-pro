    ...
    async def _execute(self, symbol: str, agent_outputs: dict = None) -> dict:
        ...
        try:
            ...
            details = {
                ...
                "price": price,
                "annual_dividend": price * dividend_yield if price and dividend_yield else None,
                "eps": eps,
                "pe_ratio": pe_ratio,
            ...
            if payout_ratio is not None:
                if payout_ratio > 100:  # Paying more than earnings
                    details["warning"] = "Dividend may not be sustainable (payout ratio > 100%)"
            ...
            data_points = sum(1 for v in [dividend_yield, price, eps, pe_ratio, payout_ratio] if v is not None)
            data_quality = (data_points / 5) * 100  # Percentage of data points available
            confidence_factors.append(data_quality)
            
            # Calculate overall confidence
            confidence = sum(confidence_factors) / len(confidence_factors) if confidence_factors else 50.0
