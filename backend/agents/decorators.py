            ...
            try:
                ...
                if cached_data:
                    try:
                        ...
                        logger.warning(
                            f"Failed to decode cached JSON for {cache_key}. Fetching fresh data."
                        )
                ...
                if result and result.get("verdict") not in ["ERROR", "NO_DATA", None]:
                    try:
                        ...
                        logger.debug(
                            f"Cached result for {cache_key} with TTL {cache_ttl}s"
                        )
                        # Potentially add cache miss/set monitoring here
                    except TypeError as json_err:
                        logger.error(
                            f"Failed to serialize result for {cache_key} to JSON: {json_err}. Result not cached."
                        )
                    except Exception as cache_err:
                        logger.error(
                            f"Failed to set cache for {cache_key}: {cache_err}. Result not cached."
                        )
                ...
                try:
                    ...
                    if result and "symbol" in result:
                        current_symbol = result[
                            "symbol"
                        ]  # Use symbol from result if available
                    ...
                    if current_symbol:  # Only update if we have a symbol
                        ...
                        logger.debug(
                            f"Tracker updated for {agent_name} ({current_symbol}): {status}"
                        )
                    else:
                        logger.warning(
                            f"Skipping tracker update for {agent_name} due to missing symbol."
                        )

                except ImportError:
                    logger.warning(
                        "Tracker module (backend.monitor.tracker) not found or get_tracker failed. Skipping tracker update."
                    )
                except AttributeError:
                    logger.warning(
                        f"Tracker instance from get_tracker() missing 'update_agent_status' method. Skipping tracker update."
                    )
                except Exception as tracker_err:
                    # Log tracker-specific errors without failing the agent execution
                    logger.warning(
                        f"Failed to update tracker for {agent_name} ({symbol}): {tracker_err}"
                    )
                ...
                logger.exception(
                    f"Error executing agent {agent_name} for symbol {symbol}: {e}"
                )
                ...
                try:
                    ...
                    if symbol:
                        ...
                        logger.debug(
                            f"Tracker updated for {agent_name} ({symbol}): {status} (after main exception)"
                        )
                    else:
                        logger.warning(
                            f"Skipping tracker update for {agent_name} after exception due to missing symbol."
                        )
                except ImportError:
                    logger.warning(
                        "Tracker module not found during exception handling. Skipping tracker update."
                    )
                except AttributeError:
                    logger.warning(
                        f"Tracker instance missing 'update_agent_status' during exception handling. Skipping tracker update."
                    )
                except Exception as tracker_err_on_fail:
                    logger.warning(
                        f"Failed to update tracker during exception handling for {agent_name} ({symbol}): {tracker_err_on_fail}"
                    )
