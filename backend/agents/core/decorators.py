        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                try:
                    ...
                    #    The decorator will then inject/override core dependencies, but only if not already provided.
                    ...
                    # But only inject if not already provided by the user.
                    needs_data_provider = ('data_provider' in func_params and 'data_provider' not in call_kwargs) or 'market_context_provider' in func_params
                    if needs_data_provider:
                        if _settings_instance: # DP requires settings to have loaded successfully
                            ...
                            # If the function explicitly asks for data_provider and it wasn't provided by user, provide it.
                            if 'data_provider' in func_params and 'data_provider' not in call_kwargs:
                    ...
                    if 'market_context_provider' in func_params:
                        ...
                        # Use user-provided data_provider if available, otherwise use the one we created
                        dp_for_mcp = call_kwargs.get('data_provider', _data_provider_instance)
                        if dp_for_mcp and _settings_instance: # MCP requires both DP and settings
                    ...
                    # Ensure agent_name and symbol are in the result (if it's a Verdict object or dict)
                    if result and isinstance(result, Verdict):
                        ...
                        # Ensure symbol is always present in the dict version
                        if "symbol" not in result_for_cache_and_dict_ops:
                            result_for_cache_and_dict_ops["symbol"] = symbol
