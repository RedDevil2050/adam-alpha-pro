...
async def run(symbol: str, agent_outputs: dict = None) -> dict: # Added agent_outputs default
    ...
    if len(log_returns) < min_days -1: # Need enough returns for the periods
        return {
            "symbol": symbol, "verdict": "NO_DATA", "confidence": 0.0, "value": None,
            "details": {"reason": f"Insufficient returns data after calculation ({len(log_returns)} points)"},
            "agent_name": agent_name
        }
    ...
    market_regime = agent_outputs.get("market_regime_agent", {}).get("verdict") if agent_outputs else "UNKNOWN"
