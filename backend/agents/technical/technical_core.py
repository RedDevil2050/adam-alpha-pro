...
def calculate_advanced_momentum(prices: pd.Series, volumes: pd.Series) -> Dict[str, float]:
...
def calculate_volatility_metrics(prices: pd.Series) -> Dict[str, float]:
    ...
    parkinsons_volatility = np.sqrt(1 / (4 * np.log(2)) * (np.log(prices.high/prices.low)**2)).mean()
