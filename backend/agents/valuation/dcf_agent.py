...
def simulate_dcf(
    base_eps: float, growth_rates: list, discount_rate: float, terminal_pe: float
) -> float:
    ...
    # Present values
    present_values = [
        eps / ((1 + discount_rate) ** (i + 1)) for i, eps in enumerate(projected_eps)
    ]
...
async def run(
    ...
    if eps is None and overview_data:
        ...
        if eps_str and eps_str.lower() not in ["none", "-", ""]:
            try:
                ...
                logger.warning(
                    f"Could not parse EPS from Alpha Vantage overview for {symbol}: {eps_str}"
                )
    ...
    if overview_data:
        ...
        if beta_str and beta_str.lower() not in ["none", "-", ""]:
            try:
                ...
                logger.warning(
                    f"Could not parse Beta from Alpha Vantage overview for {symbol}: {beta_str}"
                )
    ...
    for _ in range(n_simulations):
        ...
        perturbed_growth = [
            max(0, np.random.normal(g, 0.02))
            for g in growth_stages  # Ensure non-negative growth
        ]
        perturbed_discount = max(
            0.01, np.random.normal(discount_rate, 0.01)
        )  # Ensure positive discount rate
    ...
    if not simulation_results:
        return {
            ...
            "value": None,
            "details": {
                "reason": "All DCF simulations resulted in invalid values (e.g., discount rate <= terminal growth)"
            },
            "agent_name": agent_name,
        }
    ...
    result = {
        ...
        "details": {
            ...
            "simulation_summary": {
                ...
                "relative_std_dev": (
                    round(relative_std_dev, 4) if mean_value != 0 else None
                ),
