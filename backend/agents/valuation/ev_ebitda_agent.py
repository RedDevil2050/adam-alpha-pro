...
from backend.utils.data_provider import (
    fetch_price_point, fetch_latest_ev, fetch_latest_ebitda,
    fetch_historical_price_series, fetch_historical_ev, fetch_historical_ebitda
)
...
async def run(symbol: str, agent_outputs: dict = None) -> dict:
    ...
    try:
        ...
        hist_ebitda_task = fetch_historical_ebitda(symbol, years=ev_settings.HISTORICAL_YEARS)

        current_ev, current_ebitda, historical_ev, historical_ebitda = await asyncio.gather(
            ev_task, ebitda_task, hist_ev_task, hist_ebitda_task
        )
        ...
        return {
            ...
            "details": {"reason": f"Failed to fetch required EV/EBITDA data: {fetch_err}"},
    ...
    if current_ev is None or current_ebitda is None:
        return {
            "symbol": symbol, "verdict": "NO_DATA", "confidence": 0.0, "value": None,
            "details": {"reason": f"Missing current EV ({current_ev}) or EBITDA ({current_ebitda})"},
            "agent_name": agent_name
        }
    if current_ebitda <= 0:
        return {
            "symbol": symbol, "verdict": "NEGATIVE_OR_ZERO", "confidence": 0.7, "value": None,
            "details": {"current_ebitda": current_ebitda, "reason": "EBITDA is zero or negative"},
            "agent_name": agent_name
        }
    ...
    valid_hist_ebitda = isinstance(historical_ebitda, pd.Series) and not historical_ebitda.empty
    ...
    if valid_hist_ev and valid_hist_ebitda:
        ...
        if not common_index.empty:
            ...
            if valid_ebitda_mask.any():
                historical_ev_ebitda_series = (aligned_ev[valid_ebitda_mask] / aligned_ebitda[valid_ebitda_mask]).dropna()
                ...
                if not historical_ev_ebitda_series.empty:
                    ...
                    try:
                        ...
                        percentile_rank = stats.percentileofscore(historical_ev_ebitda_series, current_ev_ebitda, kind='rank')
                    except ImportError:
                        percentile_rank = (historical_ev_ebitda_series < current_ev_ebitda).mean() * 100

                    # Calculate Z-score
                    if std_hist_ev_ebitda and std_hist_ev_ebitda > 1e-9:
                         z_score = (current_ev_ebitda - mean_hist_ev_ebitda) / std_hist_ev_ebitda
                else:
                    logger.warning(f"[{agent_name}] Historical EV/EBITDA series empty after calculation for {symbol}")
                    data_source = "calculated_fundamental (historical calc failed)"
            else:
                logger.warning(f"[{agent_name}] No valid historical EBITDA (>0) found for {symbol}")
                data_source = "calculated_fundamental (no positive historical EBITDA)"
        else:
            logger.warning(f"[{agent_name}] No common index for historical EV and EBITDA for {symbol}")
            data_source = "calculated_fundamental (historical alignment failed)"
    else:
        logger.warning(f"[{agent_name}] Missing or invalid historical EV/EBITDA data for {symbol}")
    ...
    if percentile_rank is None:
        ...
        confidence = 0.6 + 0.3 * (1 - (percentile_rank / ev_settings.PERCENTILE_UNDERVALUED))
    elif percentile_rank >= ev_settings.PERCENTILE_OVERVALUED:
        verdict = "OVERVALUED_REL_HIST"
        confidence = 0.6 + 0.3 * ((percentile_rank - ev_settings.PERCENTILE_OVERVALUED) / (100 - ev_settings.PERCENTILE_OVERVALUED))
    else:
    ...
    details = {
        ...
        "historical_mean_ev_ebitda": round(mean_hist_ev_ebitda, 2) if mean_hist_ev_ebitda is not None else None,
        "historical_std_dev_ev_ebitda": round(std_hist_ev_ebitda, 2) if std_hist_ev_ebitda is not None else None,
