...
async def run(symbol: str, agent_outputs: dict = None) -> dict:
    ...
    cash_flow_data = await fetch_alpha_vantage("query", {"function": "CASH_FLOW", "symbol": symbol})

    if not overview_data or not cash_flow_data:
        missing_data = []
        if not overview_data: missing_data.append("overview")
        if not cash_flow_data: missing_data.append("cash_flow")
        return {
            "symbol": symbol, "verdict": "NO_DATA", "confidence": 0.0, "value": None,
            "details": {"reason": f"Could not fetch {', '.join(missing_data)} data from Alpha Vantage"},
    ...
    if market_cap_str and market_cap_str.lower() not in ["none", "-", ""]:
        try:
            ...
            if market_cap <= 0:
                 logger.warning(f"[{agent_name}] Market Cap is zero or negative for {symbol}: {market_cap_str}")
                 market_cap = None # Treat non-positive market cap as invalid for P/FCF
        except (ValueError, TypeError):
            logger.warning(f"[{agent_name}] Could not parse Market Cap for {symbol}: {market_cap_str}")
            market_cap = None
    ...
    if cash_flow_data.get("annualReports"):
        ...
        if raw_ocf_str and raw_ocf_str.lower() not in ["none", "-", ""]:
            try:
                ...
                logger.warning(f"[{agent_name}] Could not parse Operating Cash Flow for {symbol}: {raw_ocf_str}")
        ...
        if raw_capex_str and raw_capex_str.lower() not in ["none", "-", ""]:
            try:
                ...
                logger.warning(f"[{agent_name}] Could not parse Capital Expenditures for {symbol}: {raw_capex_str}")

        # Calculate FCF if both components are valid numbers
        if operating_cash_flow is not None and capital_expenditures is not None and not math.isnan(operating_cash_flow) and not math.isnan(capital_expenditures):
            fcf = operating_cash_flow - capital_expenditures
            logger.info(f"[{agent_name}] Calculated FCF for {symbol}: {fcf} (OCF: {operating_cash_flow}, CapEx: {capital_expenditures})")
        else:
             logger.warning(f"[{agent_name}] Could not calculate FCF for {symbol} due to missing/invalid OCF or CapEx.")

    # Calculate P/FCF Ratio
    pfcf_ratio = None
    if market_cap is not None and fcf is not None and fcf > 0: # FCF must be positive for a meaningful P/FCF
