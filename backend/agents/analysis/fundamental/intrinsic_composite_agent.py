...
from backend.agents.analysis.fundamental.dcf_agent import run as dcf_run
from backend.agents.analysis.fundamental.pe_ratio_agent import run as pe_run
from backend.agents.analysis.fundamental.pb_ratio_agent import (
    run as pb_run,
)  # Assuming pb_ratio_agent is also refactored
...
@standard_agent_execution(
    agent_name=agent_name, category=AGENT_CATEGORY, cache_ttl=1800
)  # Shorter TTL as it depends on others
async def run(symbol: str, agent_outputs: dict = None) -> dict:
    ...
    for i, agent_key in enumerate(tasks.keys()):
        ...
        if isinstance(result, Exception):
            logger.warning(
                f"[{agent_name}] Sub-agent {agent_key} failed for {symbol}: {result}"
            )
            continue  # Skip failed agents

        if result.get("verdict") in ["ERROR", "NO_DATA", "INVALID_DATA", None]:
            logger.warning(
                f"[{agent_name}] Sub-agent {agent_key} returned invalid verdict for {symbol}: {result.get('verdict')}"
            )
            continue  # Skip agents with no usable verdict

        verdict = result.get("verdict")
        confidence = result.get("confidence", 0.0)
        value_score = VERDICT_SCORES.get(
            verdict, 0.0
        )  # Get numerical score for the verdict
    ...
    if total_weight == 0:
        ...
        return {
            ...
            "value": None,
            "details": {
                "reason": "No valid underlying valuation signals",
