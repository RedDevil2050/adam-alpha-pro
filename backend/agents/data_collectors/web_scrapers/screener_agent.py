...
from backend.agents.data_collectors.web_scrapers.advanced_stealth_scraper import BrowserConfig
...
class ScreenerAgent(AdvancedStealthAgentBase):
    ...
    async def _fetch_primary_source(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            if hasattr(self, 'browser_enabled') and self.browser_enabled:
                ...
                channel_result = await self.stealth_scraper.quad_channel_scrape(urls[0], selectors, config=browser_config)
    ...
    async def _try_enhanced_http(self, urls: List[str], symbol: str) -> Optional[Dict]:
        ...
        for i, url in enumerate(urls):
            for attempt in range(3):  # 3 attempts per URL
                try:
                    headers = self._generate_smart_headers()
    ...
    def _generate_smart_headers(self) -> Dict[str, str]:
        ...
        accept_langs = ["en-US,en;q=0.9", "en-GB,en;q=0.9", "en-US,en;q=0.8"]
    ...
    def _extract_best_data(self, fused_data) -> Dict:
        ...
        try:
            ...
            # Handle different types of fused_data structures
            if hasattr(fused_data, 'primary'):
                # If fused_data has channel attributes
                for channel in ['primary', 'secondary', 'tertiary', 'emergency']:
                ...
                # If fused_data is a dictionary, process it directly
                for key, value in fused_data.items():
                    if value is not None:                        best_data[key] = value
            elif hasattr(fused_data, '__dict__'):
                # If fused_data is an object, get its attributes
                for key, value in fused_data.__dict__.items():
    ...
    def _calculate_confidence_score(self, data: Dict, fused_data) -> float:
        ...
        try:
            ...
            # Fusion confidence bonus - handle different fused_data types
            fusion_confidence = 0.5
            if hasattr(fused_data, 'fusion_confidence'):
                fusion_confidence = fused_data.fusion_confidence
            elif isinstance(fused_data, dict) and 'fusion_confidence' in fused_data:
                fusion_confidence = fused_data['fusion_confidence']
            
            fusion_bonus = fusion_confidence * 0.2
    ...
    async def _fetch_secondary_source(self, symbol: str) -> Optional[Dict]:
        """
        Secondary data source for Screener agent
        Fallback to Yahoo Finance and Alpha Vantage
        """
        try:
            # Use the base class's Yahoo Finance implementation
            result = await super()._fetch_secondary_source(symbol)
            # Ensure we return a dictionary
            if isinstance(result, dict):
                return result
            elif result is not None:
                return {"data": result, "source": "secondary"}
            else:
                return {"error": "No data available", "source": "secondary"}
        except Exception as e:
            self.logger.warning(f"Secondary fetch failed for {symbol}: {e}")
            return {"error": str(e), "source": "secondary"}

    async def _fetch_tertiary_source(self, symbol: str) -> Optional[Dict]:
        """
        Tertiary data source for Screener agent
        Fallback to external APIs
        """
        try:
            # Use the base class's Alpha Vantage implementation
            result = await super()._fetch_tertiary_source(symbol)
            # Ensure we return a dictionary
            if isinstance(result, dict):
                return result
            elif result is not None:
                return {"data": result, "source": "tertiary"}
            else:
                return {"error": "No data available", "source": "tertiary"}
        except Exception as e:
            self.logger.warning(f"Tertiary fetch failed for {symbol}: {e}")
            return {"error": str(e), "source": "tertiary"}

    async def _fetch_emergency_source(self, symbol: str) -> Optional[Dict]:
        """
        Emergency data source for Screener agent
        Multiple fallback sources
        """
        try:
            # Use the base class's emergency source implementation
            result = await super()._fetch_emergency_source(symbol)
            # Ensure we return a dictionary
            if isinstance(result, dict):
                return result
            elif result is not None:
                return {"data": result, "source": "emergency"}
            else:
                return {"error": "No data available", "source": "emergency"}
        except Exception as e:
            self.logger.warning(f"Emergency fetch failed for {symbol}: {e}")
            return {"error": str(e), "source": "emergency"}
