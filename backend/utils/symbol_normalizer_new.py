...
def normalize_indian_symbol(symbol: str, provider: str = "yahoo") -> str:
    """
    Convenience function to normalize Indian equity symbols.
    ...
    try:
        ...
        return normalizer.get_provider_symbol(symbol, provider)
    except Exception as e:
...
def validate_indian_symbol(symbol: str) -> Tuple[bool, str]:
    """
    Convenience function to validate Indian equity symbols.
    ...
    try:
        normalizer = IndianEquitySymbolNormalizer()
        return normalizer.validate_symbol_format(symbol)
    except Exception as e:
        logger.warning(f"Error validating symbol {symbol}: {e}")
        return False, f"Validation error: {e}"


def get_symbol_alternatives(symbol: str) -> Dict[str, str]:
    """
    Convenience function to get alternative symbol formats.
    
    Args:
        symbol: Symbol to get alternatives for
        
    Returns:
        Dict[str, str]: Alternative symbol formats
    """
    try:
        normalizer = IndianEquitySymbolNormalizer()
        return normalizer.get_alternative_symbols(symbol)
    except Exception as e:
        logger.warning(f"Error getting alternatives for symbol {symbol}: {e}")
        return {}


def analyze_symbol(symbol: str) -> Dict[str, Any]:
    """
    Convenience function to get comprehensive symbol analysis.
    
    Args:
        symbol: Symbol to analyze
        
    Returns:
        Dict[str, Any]: Complete symbol metadata and analysis
    """
    try:
        normalizer = IndianEquitySymbolNormalizer()
        return normalizer.get_symbol_metadata(symbol)
    except Exception as e:
        logger.warning(f"Error analyzing symbol {symbol}: {e}")
        return {"error": str(e), "original_symbol": symbol}
