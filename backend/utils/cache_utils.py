...
def get_redis_client() -> redis.Redis:
    ...
    # Check if we are in a testing environment *before* attempting connection
    settings = get_settings()
    if settings.is_testing:
        logger.debug("Testing environment detected. Skipping real Redis connection. Mock expected.")
        # Return None. The conftest.py fixture should patch redis_client later.
        return None
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                result = get_redis_client().get(key)
                if result:
                    return result
                # If no cached result, execute function
                result = await func(*args, **kwargs)
                # Cache the result
                get_redis_client().setex(key, ttl, str(result))
                return result
