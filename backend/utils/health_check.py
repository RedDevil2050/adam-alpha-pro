        ...
        if is_configured:
            try:
                ...
                results[api_name] = {
                    ...
                    "message": f"Successfully connected to {api_name}" if success else "Returned invalid data",
...
async def run_system_health_check() -> Dict[str, Any]:
    ...
    if not internet_check["connectivity"]:
        logger.critical("Internet connectivity check failed! System cannot function without internet.")
    ...
    working_apis = [api for api, status in api_connectivity.items() 
...
async def verify_data_providers_on_startup() -> bool:
    ...
    if not health_results["system_ready"]:
        logger.warning(f"System health check failed: {health_results.get('critical_error', 'Unknown error')}")
        logger.warning("System will start but may not be able to fetch live data!")
        return False
    
    logger.info(f"System ready with {len(health_results['working_apis'])} working APIs: {health_results['working_apis']}")
    return True