...
class MarketAnalyzer:
    ...
    def _calculate_sentiment_score(self, momentum_data: Dict) -> float:
        score = sum(momentum_data.get(k, 0) * v for k, v in self._sentiment_weights.items())
    ...
    async def get_market_statistics(self, symbols: List[str], lookback: int = 252) -> Dict[str, float]:
        await self._maybe_cleanup_cache()
        try:
            chunk_size = min(50, len(symbols))  # Ensure chunk_size doesn't exceed symbols length
            chunks = [symbols[i:i + chunk_size] for i in range(0, len(symbols), chunk_size)]
            
            # Process chunks concurrently
            chunk_tasks = [
                self._process_statistics_chunk(chunk, lookback) 
                for chunk in chunks
            ]
    ...
    async def _process_statistics_chunk(self, symbols: List[str], lookback: int) -> Dict[str, float]:
