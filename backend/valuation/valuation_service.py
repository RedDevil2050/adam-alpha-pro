...
class ValuationService:
    ...
    async def get_comprehensive_valuation(self, symbol: str) -> ValuationMetrics:
        try:
            ...
            confidence = self._calculate_confidence_score(
                fundamental, relative, technical
            )
    ...
    def _estimate_growth_rate(self, fundamentals: Dict) -> float:
        ...
        weights = [0.4, 0.3, 0.3]
        return np.average(
            [historical_growth, industry_growth, analyst_estimates], weights=weights
        )
    ...
    def _discounted_cash_flow(
        self, fcf: float, growth_rate: float, discount_rate: float, periods: int = 5
    ) -> float:
        terminal_value = (
            fcf * (1 + growth_rate) ** periods / (discount_rate - growth_rate)
        )
        dcf_value = 0
    ...
    def _calculate_confidence_score(
        self, fundamental: float, relative: float, technical: float
    ) -> float:
