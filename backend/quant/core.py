...
class QuantCore:
    ...
    @staticmethod
    def analyze_market_state(returns: pd.DataFrame, vix_data: pd.Series) -> Dict[str, float]:
    ...
    def _calculate_volatility_regime(vix: pd.Series, lookback: int = 20) -> float:
        ...
        vol_score = (current_vix - historical_mean) / vix.rolling(window=lookback).std().iloc[-1]
        return float(vol_score)

    @staticmethod
    def _calculate_momentum_score(returns: pd.Series, windows: List[int] = [5, 10, 20]) -> float:
